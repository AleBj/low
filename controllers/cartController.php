...
class cartController extends Controller
{
    ...
    public function __construct()
	{
		...
		// $this->email_test = 'alejandromarsico@gmail.com';
...
*/
    ...
    public function envioMailing($_destino, $_estado)
    {	
		...
		$_compra = $this->homeGestion->traerCompraPorId($this->_sess->get('_id_compra'));

		// echo "<pre>";print_r($_SESSION);echo "</pre>";exit;
		// echo "<pre>";print_r($this->_view->_compra);echo "</pre>";exit;


		if(($_destino == 'user' && $_compra['envio_user'] < 1) || ($_destino == 'admin' && $_compra['envio_admin'] < 1)){


			if(!empty($_compra['detalle_compra'])){

				$_total = 0;
				foreach($_compra['detalle_compra'] as $id => $val){
				...
				if(isset($_compra['descuento']) && $_compra['descuento'] > 0){
					// $this->_view->_descod = ($this->_view->_total*$_compra['descuento'])/100;
					$_descod = $_compra['descuento'];
	                $_total = $_total - $_descod;
				}
			...
			$_sucursal = $this->homeGestion->traerSucursal($_compra['id_sucursal']);
			$_grupo = $this->homeGestion->traerGrupo($_sucursal['id_grupo']);
			$_num_pedido = $_compra['id'];

			if($_destino == 'user'){

				if($_estado == 'aprobado'){
					$_mensaje = 'Tu pedido ha sido recibido.';
				}else if($_estado == 'enprogreso'){
					$_mensaje = 'Estamos procesando tu pedido.<br>A la brevedad nos contactaremos contigo.';
				}else if($_estado == 'rechazado'){
					$_mensaje = 'Su Pedido ha sido rechazado.';
				}	

			}else if($_destino == 'admin'){
				
				if($_estado == 'aprobado'){
					$_mensaje = 'Nuevo pedido recibido.';
				}else if($_estado == 'enprogreso' || $_estado == 'pendiente'){
					$_mensaje = 'Nuevo pedido pendiente.';
				}else if($_estado == 'rechazado'){
					$_mensaje = 'Nuevo pedido rechazado.';
				}
			...
			if($_destino == 'user'){
				$_email = $_user->email;
				// $_email = $this->email_test;
			}else if($_destino == 'admin'){
				$_email = 'notifications@driverschool.com.ar';
				$_copia = 'vlbozzari@driverschool.com.ar';
				$_copia2 = 'vlbozzari@gmail.com';
				$_copia_grupo = $_grupo['email'];
				// $_email = $this->email_test;
			}


			if($_estado == 'rechazado' && $_destino == 'user'){
				...
				$_body = '<!DOCTYPE html>
						...
						<head>
						    ...
						    <title>'.utf8_decode('Driver´s School').'</title>
						</head>
						<style>
						    *{
						...
						<body bgcolor="#FFFFFF" style="margin-top:0px; font-family: \'Montserrat\', sans-serif;">
						...
						<!--[if mso]>
						 <table width="600" border="0" cellpadding="0" cellspacing="0" align="center" bgcolor="#FFFFFF"><tr><td>
						<![endif]-->
						<table width="100%" border="0" cellpadding="0" cellspacing="0" align="center" bgcolor="#FFFFFF" style="max-width:600px; margin-top:0px;">
						  <tr>
						    <td>			    
						   ...
						   <table class="header" width="100%" border="0" cellpadding="0" cellspacing="0" style="height: 100px; background-color: #064482; text-align: center;">
						      <tr>
						        <td align="center" valign="top" width="100%" style="border-collapse:collapse;">
						        	<img src="'.$this->_conf['base_url'].'views/layout/default/img/logo.png" alt="Driver´s School" style="max-width: 200px;"></td>
						      </tr>
						    </table>
						   	...
						   	<div class="container">
						        ...
						        <div>
						            <h2 style="padding:20px; text-align: center;">'.$_mensaje.'</h2>
						   ...
						   <table class="header" width="100%" border="0" cellpadding="0" cellspacing="0" style="height: 100px; background-color: #064482; text-align: center;">
						      <tr>
						   	...
						   	<div class="container">
						        ...
						        <div>
						            ...
						            <div style="display:flex;justify-content: space-around;">
						                 ...
						                 <table width="100%" border="0" cellpadding="0" cellspacing="0">
						                  <tr>
						                    <td align="left" valign="top" width="4%" style="border-collapse:collapse;"></td>
						                    <td align="left" valign="top" width="46%" style="border-collapse:collapse;">
							                    <div>						                       
													<ul style="list-style: none;padding-left: 0px;">
														<li><strong>- N'.utf8_decode('ú').'mero de pedido:</strong> '.$_num_pedido.'</li>
														<li><strong>- Nombre:</strong> '.home::convertirCaracteres3($_user->nombre).' '.home::convertirCaracteres3($_user->apellido).'</li>
														<li><strong>- email:</strong> '.$_user->email.'</li>
														<li><strong>- Sucursal:</strong> '.$_sucursal['nombre'].'</li>
														<li><strong>- Fecha:</strong> '.home::formatearFecha(date('Y-m-d')).'</li>
														<li><strong>- Total:</strong> $'.number_format($_total, 2, ',', '.').'</li>
														<li><strong>- Metodo de pago:</strong> ';

														if($_compra['plataforma_pago'] == 'transfer'){
											           		$_body .= 'Transferencia bancaria directa';
											            }

											            if($_compra['plataforma_pago'] == 'mp'){

											            	if($_SESSION['_compra_response']['payment_type']=='credit_card'){
											            		$_body .= 'Mercado Pago: Tarjeta de credito';
											            	}else if($_SESSION['_compra_response']['payment_type']=='debit_card'){
											            		$_body .= 'Mercado Pago: Tarjeta de debito';
											            	}else{
											            		$_body .= 'Mercado Pago: Efectivo (pendiente de confirmaci'.utf8_decode('ó').'n del pago)';
											            	}

											            }
											            
											            if($_compra['plataforma_pago'] == 'tp'){
											            	$_body .= 'Todo Pago: '.$_SESSION['_compra_response']['Payload']['Answer']['PAYMENTMETHODNAME'];
											            }
											$_body .= '</li></ul>

							                    </div>
							                    <small>
						                            Validez de la compra 60 '.utf8_decode('días').'. <br>
						                            Una vez acreditada la compra comunicate con la sucursal seleccionada para reservar tus horarios de clase.
						                        </small>
						                    </td>
						                    
						                    <td align="left" valign="top" width="2%" style="border-collapse:collapse;"></td>
						                  </tr>
								...
								$_body .= '<!-- SOLO PARA PAGO X TRANSFERENCIA -->
						        <div style="display:flex;justify-content: space-around;">        
						            ...
						            <table width="100%" border="0" cellpadding="0" cellspacing="0">
						              <tr>
						                ...
						                <td align="left" valign="top" width="46%" style="border-collapse:collapse;">
						                    <div>					                        
										 		<p style="max-width: 100%; text-align: left; color: #064482; padding: 30px; background-color: #f1f0f4;">									 			
										 			'.strip_tags($_grupo['transferencia'], '<br>').'
										 		</p>											
						                    </div>
						                </td>
						                <td align="left" valign="top" width="2%" style="border-collapse:collapse;"></td>
						    ...
						    <div class="item" style="background-color: #f3f5f8; margin: 20px 25px; padding: 15px 30px;">
						        <h4>Detalle de compra</h4>
						        
						        <hr>

						        <div class="item-info" style="background-color: #f3f5f8;">
							        ...
							        $_body .= '<tr>						                
							                <td align="left" valign="top" width="50%" style="border-collapse:collapse;">
							                    <div>
							                        <h4><b>'.home::convertirCaracteres3(home::traerCursoPorId($val['id_curso'])->nombre).'</b></h4>
							                    </div>
							                </td>
							                <td align="right" valign="top" width="25%" style="border-collapse:collapse;">
							                    <div>
							                        <h4><b>$'.number_format($val['cantidad']*$val['precio'], 2, ',', '.').'</b></h4>
							                    </div>
							                </td>
							            </tr>';
							        }
						        ...
						        <div class="item-info2">
						            ...
						            <table width="100%" border="0" cellpadding="0" cellspacing="0">
							            <tr>
							                ...
							                <td align="right" valign="top" width="50%" style="border-collapse:collapse;">
							                    <div>
							                        <h4><b>$'.number_format($_subtotal, 2, ',', '.').'</b></h4>
							                    </div>
							                </td>
							            </tr>
							       ...
							       $_body .= '<div class="item-info2">
							            ...
							            <table width="100%" border="0" cellpadding="0" cellspacing="0">
							              <tr>
							                ...
							                <td align="right" valign="top" width="50%" style="border-collapse:collapse;">
							                    <div>
							                        <h4><b>$'.number_format($_descod, 2, ',', '.').'</b></h4>
							                    </div>
							                </td>
							                </tr>
							            </table>
						        ...
						        $_body .= '<div class="item-info2">
						            ...
						            <table width="100%" border="0" cellpadding="0" cellspacing="0">
						              <tr>
						                ...
						                <td align="right" valign="top" width="50%" style="border-collapse:collapse;">
						                    <div>
						                        <h4><b>$'.number_format($_total, 2, ',', '.').'</b></h4>
						                    </div>
						                </td>
						                </tr>
						            </table>
	        ...
	        // $this->envioMail->From = $this->_conf['smtp']['from'];
	        // $this->envioMail->FromName = utf8_decode('Driver´s School');
	        $this->envioMail->setFrom($this->_conf['smtp']['from'], utf8_decode('Driver´s School'));
	        $this->envioMail->Subject = 'Detalle de Pedido';               
	        $this->envioMail->Body = $_body;
	        $this->envioMail->addAddress($_email);     
	        /*if($_destino == 'user'){
	        	$this->envioMail->addBCC('lucianodirisio@gmail.com'); 
	        } */ 

	        if($_destino == 'admin'){
	        	$this->envioMail->addCC($_copia);
	        	$this->envioMail->addCC($_copia2);
	        	$this->envioMail->addCC($_copia_grupo);
	        	// $this->envioMail->addCC('alejandromarsico@gmail.com');
	        	// $this->envioMail->addBCC('lucianodirisio@gmail.com'); 
	        }     
	        ...
	        $this->envioMail->clearCCs();
	        $this->envioMail->clearBCCs();
	        
	        if(!$exito) {           
	            // return $this->envioMail->ErrorInfo;
	            return false;
	        }else{

	        	$_edit = contenidos_compra::find(array('conditions' => array('id = ?', $this->_sess->get('_id_compra'))));	
				if($_edit){
					if($_destino == 'user'){
						$_edit->envio_user = 1;
					}
	...
	public function procesarPaymentTest()
	{
	...
	public function procesarPaymentTesting()
	{
		...
		if($_POST){
			...
			if(validador::getPostParam('_csrf') == $this->_sess->get('_csrf')){
				...
				if(!$this->_sess->get('autenticado_front')){
					...
					\Stripe\Stripe::setApiKey($this->_conf['stripe']['private_key_qa']);
					...
					try {
					    ...
					    if($charge->outcome->network_status == 'approved_by_network'){
							...
							// echo $_resp['Messages'][0]['Status'];exit;

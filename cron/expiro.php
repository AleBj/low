...
if($_data){
    ...
    for ($i=0; $i < count($_data); $i++) { 
        ...
        $_save = $m->darBaja($_data[$i]['id'], 'close');
        if($_save){
            echo "se dio de baja<br>";
        }
...
class trabajosBase
{
    ...
    {
        $_id = (int) $_id;
        $_data = self::con()->prepare("SELECT cfa.* FROM `contenidos_forms_avisos_dos` as cfa WHERE cfa.id_form = :id");
    ...
    public function darBaja($_id, $_estado)
    {
        $_id = (int) $_id;
        // $_data = self::con()->prepare("INSERT INTO `contenidos_forms_avisos_dos` (id_form,fecha) VALUES (:id,:fecha)");
        $_data =self::con()->prepare("UPDATE `contenidos_forms_respuestas` as cfr SET cfr.estado = :estado WHERE cfr.id = :id");
        $_data->execute(array(':id' => $_id, ':estado' => $_estado));
    ...
    public function envioAviso($_email, $_nombre)
    {
                    ...
                    <body style="margin: auto;width: 50%;padding:0;box-sizing: border-box;min-width: 600px;font-family:Arial, Helvetica, sans-serif;;color: #000;">
                       ...
                       <div class="container" style="background-color: #f2f0e2; padding: 0 50px;">
                            ...
                            <div class="intro-text" >
                                ...
                                <p class="text-small" >In two days it will be six months since you chose us to support you with your Divorce. We need you to finalize your interview for filing in the next  24 hours so we can do our part!. Please complete your interview <a href="'.BASE.'user/myorders">here</a>.</p>

                                <p class="text-small" style="color: #f00;"><strongs>IMPORTANT: If you do not complete your interview by (SIX MONTH DATE from Purchase), you will no longer have access to your purchase.</strong></p>
        ...
        $_envioMail->Subject = "Please complete your Quick Divorce Interview ASAP";               
